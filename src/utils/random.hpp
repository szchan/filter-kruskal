...
struct Random {
    ...
    u64 getUint64() { return next(); }

    double getDouble() { return (next() >> 11) * (1.0 / (u64(1) << 53)); }

    float getFloat() { return (next() >> 40) * (1.0f / (u32(1) << 24)); }
    ...
    u64 getULong(u64 max) { return next() % max; }

    u64 getULong(u64 min, u64 max) { return min + getULong(max - min); }
